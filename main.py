...
def Sugador(info, email, senha, model_device):
    ...
    try:
        with httpx.Client(base_url="https://flp.distribuidoracp.com.br") as client:
            ...
            # palavras do modelo para filtro (minúsculas)
            ...
            for produto in produtos_html:
                ...
                # verifica se todas as palavras do modelo estão no nome do produto
...
def main():
    ...
    while True:
        ...
        # Corrigido: passou model_device na chamada da função Sugador
        produtos = Sugador(info, EMAIL, SENHA, model_device)
        if not produtos:
